...
router.get('/', (req, res, next) => {
    ...
    if (req.query.codUsuario){
        ...
        // Restrições de acesso à rota --- Apenas as Aplicações Pet Adote, Administradores e o Dono do recurso poderão acessar dados de endereço .
    ...
    if (req.query.codEndereco){
        ...
        // Restrições de acesso à rota --- Apenas as Aplicações Pet Adote poderão verificar um endereço específico.
            if (!req.dadosAuthToken){   // Se não houver autenticação da aplicação, não permita o acesso.
                return res.status(401).json({
                    mensagem: 'Requisição inválida - Você não possui o nível de acesso adequado para esse recurso.',
                    code: 'ACCESS_TO_RESOURCE_NOT_ALLOWED'
    ...
    // Caso 03 - Se nada for passado na Query String... Liste todos os endereços.
        ...
        // Restrições de acesso à rota --- Apenas as Aplicações Pet Adote poderão listar os endereços.
            if (!req.dadosAuthToken){   // Se não houver autenticação da aplicação, não permita o acesso.
                return res.status(401).json({
                    mensagem: 'Requisição inválida - Você não possui o nível de acesso adequado para esse recurso.',
                    code: 'ACCESS_TO_RESOURCE_NOT_ALLOWED'
...
// router.post('/', (req, res, next) => {   // A criação de um novo endereço acontecerá na rota "contas.js".
...
router.patch('/:codUsuario', async (req, res, next) => {
    ...
    // Início da verificação de existência de um endereço vínculado ao "codUsuario".
        ...
        .catch((error) => {
            console.log(`PATCH: '/usuarios/enderecos/:codUsuario' - Algo deu errado... \n`, error);
    ...
    // Início das Operações de Update.
        ...
        .catch((errorUpdate) => {
            console.log('Algo inesperado aconteceu ao atualizar os dados de endereço do usuário.', errorUpdate);
